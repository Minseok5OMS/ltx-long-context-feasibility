"""Run one frozen prompt variant with unchanged full-frame Past evidence."""
...
from five_case_prompt_common import PLAN_PATH, VARIANTS, load_plan, prompt_cache, output_path
...
def parse_args():
    ...
    p.add_argument("--cell", choices=["local", "image_past", "video_past"], required=True)
    ...
    args.reference_kind, args.position = ("none", "none") if args.cell == "local" else args.cell.split("_")
    return args
...
def main():
    ...
    plan = load_plan()
    prompt_entry = next(e for e in plan["examples"] if e["example_id"] == example.name)
    prompt = prompt_entry[VARIANTS[args.variant]]
    if metadata["prompt"] != prompt_entry["P0_original"]:
    ...
    with torch.inference_mode():
        ...
        text_cache = prompt_cache(example.name, args.variant)
        text_info = json.loads((text_cache / "inputs.json").read_text())
        if (text_info["fingerprint"]["plan_sha256"] != sha256(PLAN_PATH)
                or text_info["fingerprint"]["prompt"] != prompt
                or text_info["fingerprint"]["parent_cache_sha256"] != cache_info["inputs_sha256"]
                or text_info["fingerprint"]["models"] != models
                or sha256(text_cache / "inputs.pt") != text_info["inputs_sha256"]):
            raise ValueError("Prompt embedding cache mismatch")
        contexts = torch.load(text_cache / "inputs.pt", map_location=device, weights_only=True)
        if {k: tensor_hash(v) for k,v in contexts.items()} != text_info["tensor_hashes"]:
            raise ValueError("Actual prompt embedding hash mismatch")
        tensors.update(contexts)
        ...
        for mode in ["local" if args.cell == "local" else "oracle"]:
            if mode not in metadata["allowed_modes"]:
                raise ValueError("Mode disallowed by example metadata")
            output = output_path(example.name, args.variant, args.cell)
            output.mkdir(parents=True, exist_ok=True)
            config_path = output / "config.json"
            if config_path.exists() and json.loads(config_path.read_text()).get("status") == "completed":
                old = json.loads(config_path.read_text())
                if old["common_inputs_sha256"] != cache_info["inputs_sha256"] or old["script_sha256"] != sha256(Path(__file__)) or old["prompt_plan_sha256"] != sha256(PLAN_PATH):
                    raise ValueError(f"Completed run has different inputs/code: {output}")
                log.info("Skipping already completed %s", output)
                continue
            if config_path.exists():
                raise ValueError(f"Incomplete prior attempt must be preserved before retry: {output}")
            handler = logging.FileHandler(output / "run.log")
            logging.getLogger().addHandler(handler)
            torch.manual_seed(args.seed)
            torch.cuda.reset_peak_memory_stats(device)
            started = time.monotonic()
            ref_frames = 0 if mode == "local" else (1 if args.reference_kind == "image" else reference_frames)
            ref_offset = 0.0
            cfg = {"status": "running", "example": str(example), "example_id": example.name, "mode": mode,
                   ...
                   "reference_time_offset_seconds": ref_offset, "prompt_plan_sha256": sha256(PLAN_PATH),
                   "prompt_variant": args.variant, "prompt_text_cache": str(text_cache),
                   "prompt_text_inputs_sha256": text_info["inputs_sha256"],
                   "prompt_preparation_script_sha256": text_info["fingerprint"]["script_sha256"],
                   "common_script_sha256": sha256(ROOT / "five_case_prompt_common.py"),
                   ...
                   "prompt": prompt, "prompt_enhancement": False, "fps": fps, "width": args.width, "height": args.height,
                   ...
                   "reference_source": None if mode == "local" else {"kind": "oracle", "selection_interval": [metadata["oracle_start"], metadata["oracle_end"]],
                   ...
                   "prompt_context_hashes": {k: tensor_hash(tensors[k]) for k in ["video_context", "audio_context"]},
            ...
            try:
                ...
                cfg["reference_input_tensor_sha256"] = None
                if mode != "local":
                ...
                def denoise(transformer, video_state, audio_state, sigmas, step_index):
                    ...
                    if step_index == 0:
                        ...
                        cfg["reference_translation_audit"] = reference_shift.audit if reference_shift else None
                        before = reference_shift.before_positions if reference_shift else video_state.positions.cpu()
                        torch.save(before, output / "video_positions_before_translation.pt")
