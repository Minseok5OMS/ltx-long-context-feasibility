"""Check five-case factorial report links, UTF-8 pages and live HTTP delivery."""
...
def main():
    ...
    assert len(summary["rows"]) == 25 and audit["status"] == "passed"
    assert summary["evaluation_script_sha256"] == sha256(ROOT / "evaluate_five_case_grid.py")
    assert summary["audit_script_sha256"] == sha256(ROOT / "five_case_grid_artifacts.py")
    http = []
    if args.http:
        ...
        for path in ("reports/five_case_grid/seed42/index.html", "FIVE_CASE_GRID_RESULT.html", "FIVE_CASE_GRID_PROTOCOL.html", "report.html"):
            with urlopen(f"http://127.0.0.1:8765/{path}", timeout=15) as response:
                assert response.status == 200, path
                decoded = response.read().decode("utf-8")
                assert decoded == (ROOT / path).read_text(encoding="utf-8"), (path, "served content differs")
    record = {"status": "passed", "verified_at_utc": datetime.now(timezone.utc).isoformat(),
              ...
              "report_script_sha256": sha256(ROOT / "make_five_case_grid_report.py")}
