"""Run paired Local/Random/Oracle generation using existing LTX blocks and checkpoints."""
...
def parse_args():
    ...
    p.add_argument("--mode", choices=["local", "random", "oracle"], default="local")
    p.add_argument("--all-modes", action="store_true")
    p.add_argument("--prepare-inputs", action="store_true")
    p.add_argument("--layout", choices=["past", "target_guide"], default="past")
...
def main():
    ...
    with torch.inference_mode():
        ...
        if cache_info is None or cache_info["fingerprint"] != fingerprint:
            ...
            def encode_all(enc):
                ...
                for kind, rgb in clips.items():
                    ...
                    if kind != "local":
                        # Positive utilization control only: an earlier reference's middle still.
            ...
            cache_info = {"fingerprint": fingerprint, "inputs_sha256": sha256(cache / "inputs.pt"),
                          ...
                          "preprocessing": "Independent clips; prepend first frame; centered Lanczos crop/resize; RGB/127.5-1",
        ...
        for mode in (["local", "random", "oracle"] if args.all_modes else [args.mode]):
            if mode not in metadata["allowed_modes"]:
                raise ValueError("Mode disallowed by example metadata")
            output = ROOT / "outputs" / example.name / f"reference_{args.layout}_seed{args.seed}" / mode
            ...
            cfg = {"status": "running", "example": str(example), "example_id": example.name, "mode": mode,
                   ...
                   "reference_model_time_range": [0, reference_frames / fps] if args.layout == "past" else [(common_shift_frames + prefix_frames) / fps, (common_shift_frames + prefix_frames + 1) / fps],
                   ...
                   "reference_frames": 0 if mode == "local" else reference_frames if args.layout == "past" else 1,
            ...
            try:
                ...
                if mode != "local":
                    reference = tensors[mode if args.layout == "past" else mode + "_still"]
                    reference_index = 0 if args.layout == "past" else common_shift_frames + prefix_frames
                    conditions.append(VideoConditionByKeyframeIndex(reference, frame_idx=reference_index, strength=1.0,
                                                                    num_pixel_frames=reference_frames if args.layout == "past" else 1))
                ...
                cfg["outputs"]["target"] = write_video(output / f"local_{'only' if mode == 'local' else mode}.mp4", rgb[prefix_frames:], fps)
