"""Run an Oracle image/video × past/target time cell with the original inference recipe."""
...
def parse_args():
    ...
    p.add_argument("--reference-kind", choices=["image", "video"], required=True)
    p.add_argument("--position", choices=["past", "target"], required=True)
    ...
    args.layout = f"{args.reference_kind}_{args.position}"
    return args
...
def main():
    ...
    log = logging.getLogger("temporal_position")
    ...
    with torch.inference_mode():
        ...
        for mode in ["oracle"]:
            ...
            output = ROOT / "outputs" / example.name / f"temporal_position_seed{args.seed}" / args.layout
            output.mkdir(parents=True, exist_ok=True)
            config_path = output / "config.json"
            if config_path.exists() and json.loads(config_path.read_text()).get("status") == "completed":
                old = json.loads(config_path.read_text())
                if old["common_inputs_sha256"] != cache_info["inputs_sha256"] or old["script_sha256"] != sha256(Path(__file__)) or old["temporal_controls_sha256"] != sha256(ROOT / "temporal_controls.py"):
            ...
            ref_offset = 0 if args.position == "past" else (common_shift_frames + prefix_frames) / fps
            cfg = {"status": "running", "example": str(example), "example_id": example.name, "mode": mode,
                   ...
                   "reference_time_offset_seconds": ref_offset,
                   "reference_grid_rule": "KeyframeIndex(frame_idx=0), then translate appended temporal coordinates only",
            ...
            try:
                ...
                reference = tensors["oracle_still" if args.reference_kind == "image" else "oracle"]
                ...
                def denoise(transformer, video_state, audio_state, sigmas, step_index):
                    ...
                    if step_index == 0:
                        anchor = ROOT / "outputs" / example.name / "reference_past_seed42/oracle"
                        anchor_positions = torch.load(anchor / "video_positions.pt", map_location=device, weights_only=True)
                        if not torch.equal(video_state.positions[:, :, :base_tokens], anchor_positions[:, :, :base_tokens]):
                            raise ValueError("Base coordinates differ from the original experiment")
                        if args.reference_kind == "video":
                            expected = anchor_positions.clone()
                            expected[:, 0, base_tokens:] += ref_offset
                            if not torch.equal(video_state.positions, expected):
                                raise ValueError("Video coordinates differ from a pure translation of the original")
                        elif args.position == "target":
                            guide = ROOT / "outputs" / example.name / "reference_target_guide_seed42/oracle/video_positions.pt"
                            if not torch.equal(video_state.positions, torch.load(guide, map_location=device, weights_only=True)):
                                raise ValueError("Image target coordinates differ from the original guide")
