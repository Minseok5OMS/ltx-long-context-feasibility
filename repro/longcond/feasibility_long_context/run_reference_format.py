"""Run one reference-format diagnostic, preserving the original inference recipe."""
...
def parse_args():
    ...
    p.add_argument("--case", choices=["B", "C", "D"], required=True)
...
def main():
    ...
    from format_controls import CASES, EXAMPLE, ReferenceTimeLayout, case_path, reference_tensor
    spec = CASES[args.case]

    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    log = logging.getLogger("reference_format")
    ...
    with torch.inference_mode():
        ...
        anchors = {case: torch.load(case_path(case) / "video_positions.pt", map_location=device, weights_only=True)
                   for case in ("A", "E")}
        ...
        for case, cell in [("A", "image_past"), ("E", "video_past")]:
            ...
            if tensor_hash(anchors[case]) != record["positions_tensor_sha256"] or sha256(case_path(case) / "config.json") != record["config_sha256"]:
        ...
        for mode in ["oracle"]:
            ...
            if config_path.exists() and json.loads(config_path.read_text()).get("status") == "completed":
                ...
                if old["common_inputs_sha256"] != cache_info["inputs_sha256"] or old["script_sha256"] != sha256(Path(__file__)) or old["format_controls_sha256"] != sha256(ROOT / "format_controls.py"):
            ...
            cfg = {"status": "running", "example": str(example), "example_id": example.name, "mode": mode,
                   ...
                   "case": args.case, "reference_spec": spec, "reference_latent_temporal_frames": 10,
                   ...
                   "coordinate_anchor_config_hashes": {k: sha256(case_path(k) / "config.json") for k in ("A", "E")},
            ...
            try:
                ...
                def denoise(transformer, video_state, audio_state, sigmas, step_index):
                    ...
                    if step_index == 0:
                        expected = anchors["E"].clone()
                        if spec["layout"] == "same":
                            expected[:, 0, base_tokens:] = anchors["A"][:, 0, base_tokens:].repeat(1, 10, 1)
