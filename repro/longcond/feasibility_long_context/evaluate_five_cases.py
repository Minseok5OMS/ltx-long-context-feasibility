"""Audit ten new runs and produce comparable DINO metrics and synchronized videos."""
...
def export_comparisons(report, videos, local):
    ...
    keys = ["gt", "local", "oracle"]
    labels = {"gt": "GT (evaluation only)", "local": "Local-only", "oracle": "Local + Oracle image"}
    sequences = {k: np.concatenate([local, video]) for k, video in videos.items()}
    frames = []
    for index in range(120):
        canvas = Image.new("RGB", (1152, 248), "#141923")
        draw = ImageDraw.Draw(canvas)
        phase, t = ("local", index/24) if index < 48 else ("target", (index-48)/24)
        for col, key in enumerate(keys):
            x = col * 384
            canvas.paste(Image.fromarray(sequences[key][index]).resize((384, 216)), (x, 32))
    ...
    Image.fromarray(frames[48+32]).save(report / "comparison_poster.jpg", quality=94)
    indices = [0, 8, 16, 24, 40, 56, 71]
    sheet = Image.new("RGB", (1152, len(indices)*248), "#141923")
    for row, index in enumerate(indices):
        sheet.paste(Image.fromarray(frames[48+index]), (0, row*248))
    sheet.save(report / "comparison_frames.jpg", quality=95)
    for key in keys:
        Image.fromarray(videos[key][32]).save(report / f"{key}_poster.jpg", quality=94)
    return artifact
...
def main():
    ...
    configs, audit = audit_five_cases()
    ...
    for name in configs:
        ...
        contrasts[name] = {metric: pair["oracle"][metric]-pair["local"][metric]
                           for metric in ("dino_gt_cosine", "dino_gt_cosine_after_1s")}
    summary = {"seed": 42, "examples": 5, "new_generations": 10, "reused_generations": 0,
               "command": shlex.join([sys.executable, *sys.argv]),
               "evaluation_script_sha256": sha256(Path(__file__)), "audit_script_sha256": sha256(ROOT / "five_case_artifacts.py"),
               ...
               "rows": rows, "oracle_minus_local": contrasts, "pixel_diagnostics": diagnostics,
               "comparison_videos": comparisons, "artifact_audit": "artifact_verification.json",
               "scope": protocol["purpose"], "qualitative_review": "review.json"}
    write_json(report / "summary.json", summary)
    with (report / "results.csv").open("w", newline="", encoding="utf-8") as stream:
        writer = csv.DictWriter(stream, fieldnames=list(rows[0]))
        writer.writeheader()
        writer.writerows(rows)
    print(json.dumps({"audit": "passed", "rows": rows, "oracle_minus_local": contrasts}, ensure_ascii=False, indent=2))
